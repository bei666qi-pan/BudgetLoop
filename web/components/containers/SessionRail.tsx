...
import { Gavel, Plus, UsersRound } from "lucide-react";
  ...
  return (
    <aside className="flex min-h-0 w-full flex-col border-r border-border bg-white/60" aria-label="Sessions">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {[...sessions].sort((a, b) => Number(b.system_managed) - Number(a.system_managed)).map((session) => {
          const selected = session.id === selectedId;
          const isJudge = session.session_kind === "judge";
          return (
            <button
              key={session.id}
              type="button"
            ...
            >
              ...
              <span className="flex items-start gap-3">
                <span className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border ${isJudge ? "border-accent/30 bg-accent/10 text-accent" : selected ? "border-accent/30 bg-accent/10 text-accent" : "border-border bg-white text-muted-foreground"}`}>
                  {isJudge ? <Gavel className="h-4 w-4" /> : <UsersRound className="h-4 w-4" />}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="flex items-center justify-between gap-2">
                    <span className="truncate text-sm font-semibold text-foreground">
                      {session.role}
                      {isJudge ? <span className="ml-1.5 rounded bg-accent/10 px-1.5 py-0.5 text-[9px] font-semibold text-accent">系统必选</span> : null}
                    </span>
