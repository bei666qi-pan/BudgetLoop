...
import { ArrowRight, Clock3, Coins, LoaderCircle, Play, UsersRound } from "lucide-react";
...
export function TeamPresetPreview({ preset, roles, valid, startValid, busyAction, error, onSubmit }: TeamPresetPreviewProps) {
  ...
  return (
    <aside className="fixed inset-x-3 bottom-3 z-40 rounded-2xl border border-border bg-white/95 p-3 shadow-elevated backdrop-blur-xl xl:sticky xl:inset-auto xl:top-24 xl:z-auto xl:self-start xl:p-5">
      ...
      <div className="grid grid-cols-3 gap-2 rounded-xl bg-muted/55 p-3 text-center xl:grid-cols-1 xl:gap-3 xl:bg-transparent xl:p-0 xl:text-left">
        ...
        <div className="xl:flex xl:items-center xl:justify-between"><span className="flex items-center justify-center gap-1 text-[11px] text-muted-foreground xl:justify-start xl:text-xs"><Coins className="h-3.5 w-3.5" />Token</span><strong className="mt-1 block text-sm tabular-nums xl:mt-0">{Math.round(total.tokens / 1000)}k</strong></div>
      ...
      <p className="mt-2 hidden text-center text-[11px] leading-relaxed text-muted-foreground xl:block">创建前不会产生模型调用。所有角色保留审批与硬预算。</p>
    </aside>
  );
}
