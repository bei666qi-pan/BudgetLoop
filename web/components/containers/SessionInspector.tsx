  ...
  return (
    <aside className="min-h-0 overflow-y-auto border-l border-border bg-white/60" aria-label="团队与运行信息">
      ...
      <section className="border-b border-border p-5">
        ...
        {session.budget ? session.budget.unlimited ? <div className="mt-5 rounded-lg border border-warning/25 bg-warning/5 px-3 py-2"><p className="text-[11px] font-semibold text-warning">Max · 无自动预算上限</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">已使用 {session.budget.used_tokens.toLocaleString()} tokens · {session.budget.used_calls.toLocaleString()} 次调用</p></div> : <div className="mt-5"><div className="flex items-center justify-between text-[11px] text-muted-foreground"><span>Token 预算使用</span><span className="font-mono">{Math.round(ratio * 100)}%</span></div><div className="mt-2"><ProgressBar ratio={ratio} height="h-1.5" /></div><p className="mt-2 font-mono text-[10px] text-muted-foreground">{session.budget.used_tokens.toLocaleString()} / {session.budget.max_total_tokens.toLocaleString()} tokens</p></div> : null}
      </section>
