...
- Deliver Web, backend, macOS, and Windows from the same immutable `v0.3.0` revision.
...
- Make local `BudgetLoop.app` a freshly built `0.3.0` bundle from the released source.
...
A root `VERSION` file will contain `0.3.0`. A Python standard-library validation script will compare it with the Web and Windows `package.json`/lockfiles, backend `pyproject.toml`, macOS `Info.plist`, Tauri configuration, and Rust package metadata. CI and release workflows will run this gate.
...
- **[Unsigned or locally signed macOS app can trigger Gatekeeper]** → Document that `v0.3.0` is not Apple-notarized and provide the source-build path; do not imply notarization.
...
4. Rewrite and validate both README documents and `v0.3.0` release notes.
5. Run local backend, frontend, macOS, and release-version verification.
6. Push the verified commit to `main`, create the immutable `v0.3.0` tag, and wait for the tag workflow.
...
None for `v0.3.0`. Apple notarization and Windows code signing remain future release-hardening work.
